...
class TestPoseidonHasher:
    ...
    def test_matches_circomlib_reference_vectors(self):
        """
        Known-answer tests generated by the real `circomlibjs` npm package
        ...
        vectors = [
            ...
            (
                [0, 0, 0, 0, 0, 0, 0, 0],
                1524321216038799095937469147836866648978083754965002674906449126183454401066,
            ),
        ]
        for raw_inputs, expected in vectors:
            h = PoseidonHasher.hash([FieldElement(x) for x in raw_inputs])
            assert int(h) == expected, (
                f"Poseidon({raw_inputs}) = {int(h)}, expected {expected} "
                f"(circomlib reference vector mismatch)"
            )
